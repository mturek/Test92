	...
	for n in range(0,10):
		try:
			...
			if "User Rate Limit Exceeded" in str(error):
				print "User rate limit exceeded - Request delayed " + str(2**n) + " sec"
				time.sleep((2**n) + random.random())

	raise Exception, "Failed to load message lists from Gmail (in ListMessagesMatchingQuery)"
...
# Takes in the contents of the Sent folder and returns 
# a list of email addresses that appeared in the 'To'
# field at least once
def extractRecipients(response, user, service):
	recipient_emails = set()

	for x in response:
		msg_id = x['id']
		msg = GetMessage(service, user, msg_id)
	...
	print "Found # recipients in Sent folder:",len(recipient_emails)
